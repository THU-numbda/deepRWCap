...
from train import train_model_improved
...
TRAINING_CONFIG = {
    ...
    "tensorrt_save_dir":"/workspace/models",
...
MODELS_TO_TRAIN = [
    {
        "name": "SmallFaceSelectorWeight",
        "class": FaceSelectorWeight3D,
        "kwargs": {"name": "SmallFaceSelectorWeight"},
        "dataset": "gradient_harder",
        "target_face": None,
    },
    {
        "name": "SmallGradient1Predictor",
        "class": FacePredictor,
        "kwargs": {"name": "SmallGradient1Predictor"},
        "dataset": "gradient_harder",
        "target_face": 1,
    },
    {
        "name": "SmallGreensPredictor",
        "class": FacePredictor,
        "kwargs": {"name": "SmallGreensPredictor"},
        "dataset": "greens_harder",
        "target_face": 0,
    },
    {
        "name": "SmallFaceSelector",
        "class": FaceSelector3D,
        "kwargs": {"name": "SmallFaceSelector"},
        "dataset": "greens_harder",
        "target_face": None,
    },
    {
        "name": "SmallGradient2Predictor",
        "class": SignedFacePredictor,
        "kwargs": {"name": "SmallGradient2Predictor"},
        "dataset": "gradient_harder",
        "target_face": 2,
    }
]
...
DATASET_BASE_CONFIGS = {
    "greens_harder": {
        "path": ["/workspace/datasets/greens100k.bin"],
    ...
    "gradient_harder": {
        "path": ["/workspace/datasets/gradZ100k.bin"],
        "input_mode": "dielectric",
        "N": 23,
        "n_structures": 16,
        "dtype": np.float64
...
def benchmark_model_performance(model, input_shape=[512,1,1,23,23,23], iterations=100):
...
def compile_and_benchmark_model(model_name, model_path, config):
    ...
    try:
        # Load JIT model and move to consistent device
        ...
        print(f"  Benchmarking TorchScript model for {model_name}...")
        torchscript_latency, torchscript_throughput = benchmark_model_performance(scripted_model)
        
        if torchscript_latency is not None and torchscript_throughput is not None:
            print(f"    TorchScript: Latency={torchscript_latency:.2f}ms, "
                  f"Throughput={torchscript_throughput:.2f} samples/s")
        else:
            print(f"    TorchScript benchmarking failed - skipping TensorRT compilation")
            return
        ...
        try:
            ...
            print(f"  ❌ TensorRT FP32 failed: {str(e)[:200]}")
        ...
        if trt_fp32 is not None:
            ...
            fp32_latency, fp32_throughput = benchmark_model_performance(trt_fp32)
        ...
        if trt_fp16 is not None:
            ...
            fp16_latency, fp16_throughput = benchmark_model_performance(trt_fp16)
...
def run_compilation():
    ...
    for model_config in MODELS_TO_TRAIN:
        ...
        if os.path.exists(jit_path):
            compile_and_benchmark_model(model_name, jit_path, TRAINING_CONFIG)
        else:
...
def main():
    ...
    if not provided_args.issubset(valid_args):
        ...
        print("Usage: python script.py [train] [compile]")
    ...
    if len(args) == 0:
        ...
        if "train" in args:
            ...
            print("🚀 Running compilation only")
            run_compilation()
    elif len(args) == 2 and "train" in args and "compile" in args:
